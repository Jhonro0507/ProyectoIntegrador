...
public class VueloServiceTest {
    ...
    @Test
    public void testObtenerVuelosDisponiblesPorFechaSuccess() {
        // Arrange
        LocalDate fecha = LocalDate.now();
        List<Vuelo> vuelosDisponibles = Arrays.asList(new Vuelo());
        when(vueloRepository.findVuelosDisponiblesPorFecha(fecha)).thenReturn(vuelosDisponibles);

        // Act
        ResponseEntity<?> responseEntity = vueloService.obtenerVuelosDisponiblesPorFecha(fecha);

        // Assert
        verify(vueloRepository, times(1)).findVuelosDisponiblesPorFecha(fecha);
        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        Assertions.assertEquals(vuelosDisponibles, responseEntity.getBody());
    }

    @Test
    public void testObtenerVuelosDisponiblesPorFechaNotFound() {
        // Arrange
        LocalDate fecha = LocalDate.now();
        when(vueloRepository.findVuelosDisponiblesPorFecha(fecha)).thenReturn(Collections.emptyList());
        ...
        verify(vueloRepository, times(1)).findVuelosDisponiblesPorFecha(fecha);
        Assertions.assertEquals(HttpStatus.NOT_FOUND, responseEntity.getStatusCode());
        Assertions.assertEquals("No hay Vuelos disponibles para la fecha especificada.", responseEntity.getBody());
    }

    @Test
    public void testObtenerVuelosDisponiblesPorFechaFailure() {
        // Arrange
        LocalDate fecha = LocalDate.now();
        when(vueloRepository.findVuelosDisponiblesPorFecha(fecha)).thenThrow(new RuntimeException("Test Exception"));
    ...
    @Test
    public void testObtenerVuelosDisponiblesPorFechaYTipoSuccess() {
        // Arrange
        LocalDate fecha = LocalDate.now();
        String tipo = "TipoEjemplo";
        List<Vuelo> vuelosDisponibles = Arrays.asList(new Vuelo(/* provide necessary vuelo data */));
        when(vueloRepository.findVuelosDisponiblesPorFechaYTipo(fecha, tipo)).thenReturn(vuelosDisponibles);

        // Act
        ResponseEntity<?> responseEntity = vueloService.obtenerVuelosDisponiblesPorFechaYTipo(fecha, tipo);

        // Assert
        verify(vueloRepository, times(1)).findVuelosDisponiblesPorFechaYTipo(fecha, tipo);
        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        Assertions.assertEquals(vuelosDisponibles, responseEntity.getBody());
    }

    @Test
    public void testObtenerVuelosDisponiblesPorFechaYTipoNotFound() {
        // Arrange
        LocalDate fecha = LocalDate.now();
        String tipo = "TipoEjemplo";
        when(vueloRepository.findVuelosDisponiblesPorFechaYTipo(fecha, tipo)).thenReturn(Collections.emptyList());
        ...
        verify(vueloRepository, times(1)).findVuelosDisponiblesPorFechaYTipo(fecha, tipo);
        Assertions.assertEquals(HttpStatus.NOT_FOUND, responseEntity.getStatusCode());
        Assertions.assertEquals("No hay Vuelos disponibles para la fecha y tipo especificados.", responseEntity.getBody());
    }

    @Test
    public void testObtenerVuelosDisponiblesPorFechaYTipoFailure() {
        // Arrange
        LocalDate fecha = LocalDate.now();
        String tipo = "TipoEjemplo";
        when(vueloRepository.findVuelosDisponiblesPorFechaYTipo(fecha, tipo)).thenThrow(new RuntimeException("Test Exception"));
